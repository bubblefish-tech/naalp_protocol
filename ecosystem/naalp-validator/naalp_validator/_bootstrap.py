...
"""
Repo-relative import bootstrap for the semantic validator.

This package REUSES the Part-1 Python reference SDK (`impl/python/naalp`) rather than
reimplementing the CBOR codec, the object envelope, or the channel/kind registry (the
repository's non-negotiable discipline: no second object encoding, no duplicated wire
truth). The `naalp` package is not installed (no `pip install -e`) in this checkout, so
this module locates `impl/python` by walking upward from this file -- never an absolute
path -- and inserts it onto `sys.path` exactly once, before anything imports `naalp`.

The walk looks for a directory that contains `impl/python/naalp/__init__.py`; this is
robust to the exact nesting depth of `ecosystem/naalp-validator/` and mirrors the
existing `_find_vector()` upward-walk convention used by
`impl/python/tests/test_audience.py`.
"""
import os
import sys
...
def ensure_naalp_on_path():
    ...
    if impl_python is None:
        raise NaalpSdkNotFound(
            ...
            "the naalp-validator package must live inside the naalp_draft-01 tree" % here
        )
