...
| artifact | path |
|---|---|
| Internet-Draft source (kramdown-rfc) | `ietf/draft-bubblefish-naalp-01.md` |
| IANA registration package | `ietf/IANA.md` |
| Submission runbook (datatracker + ISE + IANA) | `ietf/SUBMISSION.md` |
| Normative CDDL | Appendix A of the draft (byte-identical mirror: `spec/naalp-draft-01.cddl`) |

## Building the draft

```console
$ cd ietf
$ scripts/regen_ietf_draft.sh   # kramdown-rfc -> xml2rfc --v2v3 -> post-process -> clean v3 renders
```
...
3. Email the Independent Submissions Editor (`rfc-ise@rfc-editor.org`) with the checklist,
   ...
   (Stage 2).
4. IANA registrations — the media type `application/vnd.bubblefish.naalp+cbor` (RFC 6838) and the eight N-AALP
   registries (RFC Required / FCFS), all carried in the draft's IANA Considerations (Stage 3).
