...
- **Put every MCP tool call behind one command.** [`naalp-mcp-guard`](ecosystem/naalp-mcp-guard/) fronts any MCP server — `wrap` it, then `verify` — effect-classifying each tool call, gating non-idempotent and destructive effects on a single-use human approval bound to the exact `(tool, arguments)`, and emitting a post-quantum-signed, offline-verifiable receipt per effect. It does not un-poison a tool description or detect a prompt injection — but with the guard as the sole path to the server, a fooled agent still **cannot execute an unauthorized consequential effect**, and every attempt is receipted. *(Reference CLI; install from source — PyPI packaging pending.)*
- **Governance, provenance and accountability as first-class objects.** N-AALP makes the signed **object**, not the connection, the unit of security: a closed four-value **effect** label that is an authorization input (not a hint), single-use **approvals** bound to the exact action, an append-only **audit chain** with signed equivocation (fork) proofs, and monotone-attenuation **delegation** — decision-lineage, provenance, and delegation accountability, verifiable offline on any wire. *(Effect, approval, and audit graded across all ten ports; delegation and its monotone-attenuation invariant graded in the Go/Rust reference and machine-proven.)*
- **Post-quantum by default, ahead of the deadlines.** Every object is a COSE_Sign1 signed with ML-DSA-65/-87 ([FIPS 204](https://csrc.nist.gov/pubs/fips/204/final), deterministic) with an optional Ed25519 hybrid leg, plus a non-decomposable LAMPS composite so neither leg can be stripped. Long-lived receipts, approvals, and audit chains stay safe against store-now-verify-later forgery. *(Graded against non-circular FIPS-204 / RFC-8032 oracles across all ten ports.)*
- **Bounded autonomy that plugs into the framework you already run.** A typed effect contract + fail-closed checks + a human-in-the-loop interceptor (blocking, durable across restarts, approve / reject / **edit**), wired into LangGraph (gating on the real `interrupt()`), OpenAI Agents, Microsoft Agent Framework, CrewAI, LlamaIndex, and Google ADK — each a thin adapter over one neutral core. *(Adapters graded with mutation evidence.)*
- **Bridge MCP and A2A without re-architecting.** Carry MCP and A2A traffic octet-exact inside a governed, signed envelope — as an in-process hook or a sidecar/gateway — so a bridged payload inherits N-AALP identity, effect, approval, and audit with no re-encoding. Absent effect on a mutating carried request fails closed to `destructive`. *(Graded carriage; the MCP binding profile is normative.)*
- **First-class agent identity and delegation.** The signer identity is a pure function of the public key (verify offline, no directory, no lookup), with co-signed key rotation, revocation, foreign-identity linkage, and multi-hop DelegationGrant chains whose descendants can never hold more authority than their parent. *(Graded; a GNAP ([RFC 9635](https://www.rfc-editor.org/rfc/rfc9635)) delegation bridge is built.)*
- **Recomputable, offline-verifiable evidence.** Single-use consume receipts prove exactly-once execution; offline proof bundles package an object with every verification input; and RFC-4998-class evidence renewal re-anchors an aging signature so non-repudiation survives decades — an independent party recomputes the verdict without trusting the operator. *(Receipts and bundles graded; evidence renewal reference-graded in Go and Rust.)*
- **Governance for physical AI and manufacturing.** A closed physical-hazard authorization class — orthogonal to effect, requiring exact-class **and** full-containment authorization or the whole claim is denied — plus a stable C-ABI controller signer (with a `no_std` bare-metal subset) and an OPC UA carriage binding. The hazard extension is wire-frozen and graded byte-identical across all ten ports. *(The controller signer, OPC UA carriage, and hazard extension are graded; the packaged SDK entry point is documentation composing them; device-identity and runtime-assurance components are deferred pending hardware.)*
- **Standards-anchored, byte-identical, provable.** Ten reference SDKs graded against one 565-case corpus whose every expected value traces to an RFC / FIPS / NIST vector — never to the code under test; Go and Rust are byte-identical; and the core security invariants carry machine-checked Alloy + TLA+/Apalache proofs. *(CI grades all of it on every push.)*

---

## What's New

*Recent additions, current as of 2026-09-07. The SDKs are pre-1.0 (`0.y.z`); every change below lives under [CHANGELOG.md](CHANGELOG.md) `[Unreleased]` and will receive a version on the first release tag. Dates are the git last-updated date of each addition.*

- **draft-01 submission-ready wave** — a signed `audience` field with fail-closed `WrongAudience` verification; the non-decomposable LAMPS composite signature (neither leg can be stripped); the full 75-kind channel/effect table and a named error-code registry; MUST-level decoder resource bounds; determinism hardening; and total event×state tables for all four object state machines, graded by deviant-trace + learned-model-diff suites. (See [CHANGELOG.md](CHANGELOG.md) `[Unreleased]`.)
- **One-command MCP security guard** — [`naalp-mcp-guard`](ecosystem/naalp-mcp-guard/): `wrap` / `verify` a foreign MCP server with effect-gating, single-use human approval, and a PQ-signed per-effect receipt. *(Reference CLI; PyPI packaging pending; consequence-closed for tool-poisoning — the trick is not prevented, the unauthorized effect is.)*
- **Framework governance adapters** — the LangGraph landmark adapter (gating on the real `interrupt()`), plus OpenAI Agents, Microsoft Agent Framework, CrewAI, and LlamaIndex adapters, and the durable HITL interceptor. *(Graded with mutation evidence.)*
- **Manufacturing / Physical-AI evidence components** — the wire-frozen physical-hazard authorization extension (10-port byte-identical), the C-ABI and `no_std` controller signers, the OPC UA carriage binding, and the packaged SDK entry point ([`ecosystem/naalp-manufacturing-sdk/`](ecosystem/naalp-manufacturing-sdk/)). *(Components A/B/F graded; SDK entry point is documentation; device-identity/runtime-assurance deferred pending hardware.)*
- **Ecosystem bridges** — a DID/VC signer-identity bridge (`did:jwk`) and a GNAP (RFC 9635) delegation bridge, plus a Go SBOM + signed-provenance supply-chain package. *(Built / unit-tested.)*
- **Machine-checked formal proofs** — a bounded Alloy proof and an unbounded-method TLA+/Apalache inductive proof of the monotone-attenuation invariant, plus a TLA+ consume-ledger (exactly-once) model with fault fixtures ([`formal/`](formal/)).
- **Curated public conformance evidence** — a conformance matrix and per-package engineering-rigor pages in RFC-7942 vocabulary ([`docs/conformance-matrix.md`](docs/conformance-matrix.md), [`docs/engineering-rigor/`](docs/engineering-rigor/)).
